...
CREATE VIEW top_salarios AS SELECT nDIEmp FROM empleados order by nDIEmp desc limit 5;

-- 2. Crear vista que muestre los 10 empleados con más antigüedad en la empresa, mostrando el nombre del empleado y departamento y el año. Todos empleados pertenezcan o no a algún departamento.

create view empleados_antiguos as select empleados.nomEmp, derpartamentos.nombreDpto, empleados.fecIncorporacion from empleados, departamentos where empleados.codDpto = departamentos = cod.Depto order by empleados.fecIncorporacion asc limit 10;
