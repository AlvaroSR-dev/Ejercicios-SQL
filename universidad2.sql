...
create table numero_alumnos_curso select curso_escolar.id as curso, count(alumno_se_matricula_asignatura.id_alumno) as numero from alumno_se_matricula_asignatura, curso_escolar where alumno_se_matricula_asignatura.id_curso_escolar = curso_escolar.id group by alumno_se_matricula_asignatura.id_curso_escolar;
select * from numero_alumnos_curso;

/* Ejercicio 2 */
select * from persona;
select persona.nombre from persona, profesor where persona.id = profesor.id_profesor and persona.sexo = 'M';
...
delete persona from persona where persona.fecha_nacimiento like '2000______';

/* Ejercicio 4 */

create view alumno_y_creditos as select persona.nombre as nombre, sum(asignatura.creditos) as creditos from persona inner join alumno_se_matricula_asignatura on persona.id = alumno_se_matricula_asignatura.id_alumno inner join asignatura on alumno_se_matricula_asignatura.id_asignatura = asignatura.id group by persona.nombre;
 
 /* Ejercicio 5 */	

create view profesores_primer_curso as select distinct(persona.id) as id_profesor, persona.nombre as nombre, asignatura.nombre as asignatura, asignatura.curso from persona inner join asignatura on persona.id = asignatura.id_profesor where asignatura.curso = '1' group by persona.id;
select * from profesores_primer_curso;

/* Ejercicio 6 */

create view personal_docente as select persona.id as id_profesor, persona.nombre as nombre, persona.apellido1 as apellido from persona inner join profesor on profesor.id_profesor = persona.id;
select * from personal_docente