...
select nombre from asignatura where id_profesor = 3 and cuatrimestre = 1;

/* 2. Tabla que muestre el id y el nombre de las asignaturas que tengan 6 créditos y sean
básicas, o sean del grado con id 4 */

select id, nombre from asignatura where creditos = 6 and tipo = 1 or id = 4;

/* 3. Id y nombre de la asignatura que tiene el nombre más largo. */

select id, nombre from asignatura order by char_length(nombre) desc limit 1;
...
select nombre, apellido1, apellido2, fecha_nacimiento from persona order by fecha_nacimiento desc limit 5;
/* 6. Número de personas que han nacido después del 1 de Enero de 1977. */

select count(nombre) from persona where fecha_nacimiento > '1997-01-01';

/* 7. Lista que muestre cuántos hombres y cuántas mujeres hay. */

select sexo , count(sexo) as Número from persona group by sexo;

/* 8. Tabla que muestre el nombre de las personas, teléfono y una columna llamada ‘phone’
de los últimos 4 números excluyendo los nulos. */
...
select departamento.nombre from departamento, profesor where departamento.id = profesor.id_departamento and profesor.id_profesor is not null group by departamento.nombre;

/* 10. Nombre, teléfono de los profesores que dan más de 2 asignaturas. */

select persona.nombre, persona.telefono from profesor, persona where (id_profesor > 2) and persona.tipo like 'profesor' group by (nombre);

/* 11. Nombre de las asignaturas del grado ‘Grado en Química (Plan 2009)’ */

select asignatura.nombre from asignatura,grado where asignatura.id_grado = grado.id and grado.nombre like 'Grado en Química (Plan 2009)';

/* 12. Toda información de las personas que no están matriculadas en ninguna asignatura. */

select persona.* from persona, alumno_se_matricula_asignatura where persona.id = alumno_se_matricula_asignatura.id_alumno and alumno_se_matricula_asignatura.id_asignatura = null;

/* 13. Todas asignaturas que no tienen alumnos. */

select asignatura.nombre from asignatura, alumno_se_matricula_asignatura where asignatura.id = alumno_se_matricula_asignatura.id_asignatura and alumno_se_matricula_asignatura.id_alumno = null;

/* 14. Nombre de las asignaturas que tienen más de 5 alumnos. */

select asignatura.nombre from asignatura, alumno_se_matricula_asignatura where asignatura.id = alumno_se_matricula_asignatura.id_asignatura and alumno_se_matricula_asignatura.id_alumno > 5;