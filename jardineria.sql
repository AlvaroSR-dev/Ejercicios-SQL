...
select count(nombre) from producto where precio_venta > (select avg(precio_venta) from producto);

select nombre from producto where gama like 'Frutales';

select gama, count(nombre) from producto group by gama having count(nombre) > 3 ;

select count(codigo_producto) from detalle_pedido where (select count(codigo_producto) from detalle_pedido) > 0;

select cliente.*, pago.total from cliente, pago where cliente.codigo_cliente = pago.codigo_cliente order by pago.total desc limit 1;

select cliente.nombre_cliente, pago.forma_pago from cliente, pago where cliente.codigo_cliente = pago.codigo_cliente;

select distinct cliente.*, sum(pago.total) from cliente, pago where cliente.codigo_cliente = pago.codigo_cliente group by pago.codigo_cliente order by sum(pago.total) limit 1;
select cliente.*, (select sum(pago.total) from pago, cliente having cliente.codigo_cliente = pago.codigo_cliente) from cliente, pago where cliente.codigo_cliente = pago.codigo_cliente group by cliente.nombre_cliente order by (select sum(pago.total) from pago, cliente having cliente.codigo_cliente = pago.codigo_cliente);

select producto.nombre from producto, detalle_pedido, pedido, cliente, pago where producto.codigo_producto = detalle_pedido.codigo_producto and detalle_pedido.codigo_pedido = pedido.codigo_pedido and pedido.codigo_cliente = cliente.codigo_cliente and cliente.codigo_cliente = pago.codigo_cliente and pedido.fecha_pedido between  '2008-01-01' and '2008-12-31' and (select pago.total from pago order by pago.total desc limit 1);

select cliente.nombre_cliente from cliente, pedido, detalle_pedido, producto where cliente.codigo_cliente = pedido.codigo_cliente and pedido.codigo_pedido = detalle_pedido.codigo_pedido and detalle_pedido.codigo_producto = producto.codigo_producto and producto.proveedor like 'Murcia Seasons' group by cliente.nombre_cliente;

select nombre, gama from producto where gama is null and nombre is null;

/* Ejercici 2 */

INSERT INTO oficina VALUES ('ALM-ES','Almería','España','Almería','04001','+34 92 264 245','Calle Estocolmo','1B');

INSERT INTO empleado VALUES (32,'Andrés','Iniesta','Zamarreño','2442','iniesta@jardineria.es','ALM-ES',NULL,'Representante Ventas');

INSERT INTO cliente VALUES (39,'CES Juan Pablo II','Jorge','Pérez','635598799','918735678','C/Las Flores','Almería','Almería','Almería','Spain','04001',32,40000);

UPDATE cliente codigo_cliente SET codigo_cliente = 41 where codigo_cliente = 39;
